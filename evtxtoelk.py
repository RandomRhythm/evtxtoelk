...
if __name__ == "__main__":
    ...
    parser.add_argument('-movefolder', default="", help="Move completed evtx files to this folder")

    # Parse arguments and call evtx to elk class
    args = parser.parse_args()
    if os.path.isdir(args.evtxfile):  
        for (dirpath, dirnames, filenames) in os.walk(args.evtxfile):
          for file in filenames:
              scanPath = os.path.join(dirpath, file)
              print(scanPath + "\n")
              EvtxToElk.evtx_to_elk(scanPath, args.elk_ip, elk_index=args.i, bulk_queue_len_threshold=int(args.s), metadata=args.meta)
              if args.movefolder != "":
                  os.replace(scanPath, os.path.join(args.movefolder, file))
 
    elif os.path.isfile(args.evtxfile):  
